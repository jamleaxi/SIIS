...
class CommonSupplyRequestIssuedController extends Controller
{
    public function index()
    {
        if  (Auth::id()) // checks if the user is logged in
        {
            ...
            if ($status == "ACTIVE")
            {
                ...
                {
                    return view('admin.common-supply-request-issued'); // go to admin index
                }
                elseif($role == 'User')    // if the user is just a user
                {
                    return view('user.common-supply-request-issued');  // go to user index
                }
            }
