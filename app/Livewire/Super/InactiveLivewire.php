...
class InactiveLivewire extends Component
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    public function render()
    {
        ...
        {
            $allusers = User::where('status','=','INACTIVE')->orderBy('name','asc')->paginate($this->pagination);
            $this->count = count(User::where('status','=','INACTIVE')->get());
        ...
        else
        {
            ...
            $allusers = User::where('status','=','INACTIVE')
                ->where(function($query) use($search){
                    $query->where('name','like','%'.$search.'%')
                        ->orWhere('email','like','%'.$search.'%');
                })->orderBy('name','asc')->paginate($this->pagination);

            $this->resultCount = User::where('status','=','INACTIVE')
                ->where(function($query) use($search){
                    $query->where('name','like','%'.$search.'%')
                        ->orWhere('email','like','%'.$search.'%');
        ...
        return view('livewire.super.inactive-livewire',[
            'allusers' => $allusers,
            'positions' => $positions,
            'offices' => $offices,
