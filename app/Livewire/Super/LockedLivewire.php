...
class LockedLivewire extends Component
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    public function render()
    {
        ...
        {
            $allusers = User::where('status','=','LOCKED')->orderBy('name','asc')->paginate($this->pagination);
            $this->count = count(User::where('status','=','LOCKED')->get());
        ...
        else
        {
            ...
            $allusers = User::where('status','=','LOCKED')
                ->where(function($query) use($search){
                    $query->where('name','like','%'.$search.'%')
                        ->orWhere('email','like','%'.$search.'%');
                })->orderBy('name','asc')->paginate($this->pagination);

            $this->resultCount = User::where('status','=','LOCKED')
                ->where(function($query) use($search){
                    $query->where('name','like','%'.$search.'%')
                        ->orWhere('email','like','%'.$search.'%');
        ...
        return view('livewire.super.locked-livewire',[
            'allusers' => $allusers,
            'positions' => $positions,
            'offices' => $offices,
