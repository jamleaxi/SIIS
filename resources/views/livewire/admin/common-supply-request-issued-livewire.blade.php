                                        ...
                                        <div class="col-12 col-sm-12">
                                            ...
                                            <div class="card-header p-0 pt-1">
                                                <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
                                                    ...
                                                    <li class="nav-item">
                                                        <a class="nav-link active" id="custom-tabs-two-issued-tab" data-toggle="pill" href="#custom-tabs-two-issued" role="tab" aria-controls="custom-tabs-two-issued" aria-selected="true"><i class="fas fa-thumbs-up mr-2"></i>Issued</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="card-body">
                                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                                    <div class="tab-pane fade show active" id="custom-tabs-two-issued" role="tabpanel" aria-labelledby="custom-tabs-two-issued-tab">
                                                        {{-- issued requests --}}
                                                        @if(count($issuances) != 0)
                                                            <div class="timeline">
                                                                @foreach ($unique_dates_iss as $unique_date_iss)
                                                                    <div class="time-label">
                                                                        <span class="bg-yellow">&nbsp;{{ \Carbon\Carbon::parse($unique_date_iss->date_iss)->format('F j, Y') }}&nbsp;</span>
                                                                    </div>

                                                                    @foreach ($issuances->where('date_iss','=',$unique_date_iss->date_iss) as $issuance)
                                                                        <div>
                                                                            @if($issuance->date_rec)
                                                                            ...
                                                                            <div class="timeline-item">
                                                                                <span class="time"><i class="fas fa-clock"> {{ $issuance->created_at }} ({{ \Carbon\Carbon::parse($issuance->created_at)->diffForHumans(\Carbon\Carbon::now(),true) }} ago)</i></span>

                                                                                @if ($issuance->status == 'COMPLETED')
                                                                                    <h3 class="timeline-header">
                                                                                        <a class="text-green mr-1">
                                                                                            @if($issuance->date_rec)
                                                                                                <span class="badge bg-green">Issuance confirmed by requester</span>
                                                                                            @else
                                                                                                <span class="badge bg-red">Issuance not yet confirmed by requester</span>
                                                                                            @endif

                                                                                            @if($issuance->type == 'standard')
                                                                                                <span class="badge bg-indigo">Standard</span>
                                                                                            @else
                                                                                                <span class="badge bg-fuchsia">Project</span>
                                                                                            @endif

                                                                                            {{ $issuance->status }}: {{ $issuance->division }} - {{ $issuance->office }} <span class="text-gray">| <i class="fas fa-ticket-alt"></i> {{ $issuance->tcode }}</span>
                                                                                        </a>
                                                                                    </h3>
                                                                                @endif
                                                                                ...
                                                                                <div class="timeline-body">
                                                                                    <div class="row mb-2">
                                                                                        <div class="col-12 mb-1">
                                                                                            <b class="text-gray">Purpose: </b>{{ $issuance->purpose }}
                                                                                        </div>

                                                                                        <div class="col-6">
                                                                                            @foreach($system_users->where('id','=',$issuance->requester_id) as $system_user_req)
                                                                                                <img src="{{ $system_user_req->profile_photo_url }}" class="img-circle mr-1" width="20" height="20">
                                                                                                <b class="text-gray">Requester:</b>
                                                                                                <b>{{ $issuance->requester }} - {{ $issuance->position_req }}</b>
                                                                                            @endforeach
                                                                                        </div>
            
                                                                                        <div class="col-6">
                                                                                            @foreach($system_users->where('id','=',$issuance->approver_id) as $system_user_app)
                                                                                                <img src="{{ $system_user_app->profile_photo_url }}" class="img-circle mr-1" width="20" height="20">
                                                                                                <b class="text-gray">Approver:</b>
                                                                                                <b>{{ $issuance->approver }} - {{ $issuance->position_app }}</b>
                                                                                            @endforeach
                                                                                        </div>

                                                                                        @if ($issuance->assessor_id != '0')
                                                                                            <div class="col-6">
                                                                                                @foreach($system_users->where('id','=',$issuance->assessor_id) as $system_user_ass)
                                                                                                    <img src="{{ $system_user_ass->profile_photo_url }}" class="img-circle mr-1" width="20" height="20">
                                                                                                    <b class="text-gray">Assessor: </b>
                                                                                                    <b>{{ $issuance->assessor }} - {{ $issuance->position_ass }}</b>
                                                                                                @endforeach
                                                                                            </div>
                                                                                        @endif

                                                                                        @if ($issuance->issuer_id != null or $issuance->issuer_id != '')
                                                                                            <div class="col-6">
                                                                                                @foreach($system_users->where('id','=',$issuance->issuer_id) as $system_user_iss)
                                                                                                    <img src="{{ $system_user_iss->profile_photo_url }}" class="img-circle mr-1" width="20" height="20">
                                                                                                    <b class="text-gray">Issuer: </b>
                                                                                                    <b>{{ $issuance->issuer }} - {{ $issuance->position_iss }}</b>
                                                                                                @endforeach
                                                                                            </div>
                                                                                        @endif

                                                                                        @if ($issuance->receiver_id != null or $issuance->receiver_id != '')
                                                                                            <div class="col-6">
                                                                                                @foreach($system_users->where('id','=',$issuance->receiver_id) as $system_user_rec)
                                                                                                    <img src="{{ $system_user_rec->profile_photo_url }}" class="img-circle mr-1" width="20" height="20">
                                                                                                    <b class="text-gray">Receiver: </b>
                                                                                                    <b>{{ $issuance->receiver }} - {{ $issuance->position_rec }}</b>
                                                                                                @endforeach
                                                                                            </div>
                                                                                        @endif
                                                                                    </div>
                                                                                    ...
                                                                                    <div class="row border-top">
                                                                                        <div class="col-12 mt-2">
                                                                                            @if ($issuance->status == 'COMPLETED')
                                                                                                <button class="btn bg-blue btn-sm" style="border-radius: 20px" wire:click='viewGeneratedRIS({{ $issuance->id }})' wire:loading.attr="disabled"><i class="fas fa-file-alt mr-1"></i>View Accomplished RIS</a>
                                                                                                @if($issuance->date_rec)
                                                                                                    <button class="btn bg-maroon btn-sm" style="border-radius: 20px" wire:click='openRIS({{ $issuance->id }})' wire:loading.attr="disabled"><i class="fas fa-file-pdf mr-1"></i>View as PDF file</button>
                                                                                                @endif
                                                                                            @endif
                                                                                        </div>
                                                                                    </div>
                                                        ...
                                                        @else
                                                            <i class="fas fa-info-circle mr-2"></i>There are no issued requests as of the moment.
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
...
@script
    <script>
        ...
        $('#nav-request-issued').addClass('active');
