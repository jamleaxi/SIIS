                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mt-3">
                            <div class="card card-dark">
                                <div class="card-header">
                                    <h3 class="card-title"><i class="fas fa-user-secret mr-3"></i>Profile View</h3>
                        ...
                        @elseif($show_edit == true)
                            <div class="col-md-12 mt-3">
                                <div class="card card-blue">
                                    <div class="card-header">
                                        <h3 class="card-title"><i class="fas fa-user-secret mr-3"></i>Profile Update</h3>
                        ...
                        @else
                            <div class="col-md-12 mt-3">
                                <div class="card card-dark">
                                    <div class="card-header">
                                        <h3 class="card-title"><i class="fas fa-user-secret mr-3"></i>Superadmins ({{ $count }})</h3>
                                    ...
                                    <div class="card-body">
                                            ...
                                            <tbody>
                                                @foreach($superadmins as $superadmin)
                                                    <tr>
                                                        ...
                                                        <td><img src="{{ $superadmin->profile_photo_url }}" class="img-circle" width="25" height="25"></td>
                                                        <td>
                                                            @if($superadmin->id == $my_id)
                                                                <b>{{ $superadmin->name }} (You)</b>
                                                            @else
                                                                {{ $superadmin->name }}
                                                            @endif
                                                        </td>
                                                        <td>{{ $superadmin->email }}</td>
                                                        <td>{{ $superadmin->office }} ({{ $superadmin->office_full }})</td>
                                                        <td>{{ $superadmin->position }} ({{ $superadmin->position_full }})</td>
                                                        <td>
                                                            @if($superadmin->status == "ACTIVE")
                                                                <div class="text-success"><i class="fas fa-user-check mr-1"></i>Active</div>
                                                            @elseif($superadmin->status == "INACTIVE")
                                                        ...
                                                        <td>
                                                            <a class="btn btn-xs bg-primary" wire:click="viewSuperadmin({{ $superadmin->id }})"><i class="fas fa-eye" style="cursor: pointer"></i></a>
                                                            <a class="btn btn-xs bg-purple" wire:click="editSuperadmin({{ $superadmin->id }})"><i class="fas fa-building-user" style="cursor: pointer"></i></a>

                                                            @if($count > 1 and $superadmin_active > 1)
                                                                <a class="btn btn-xs bg-pink" wire:click.prevent="setSuperRemove({{ $superadmin->id }})"><i class="fas fa-user-minus" style="cursor: pointer"></i></a>
                                                            @endif

                                                            @if($superadmin->id == $my_id)
                                                                @if($superadmin->status == "ACTIVE")
                                                                    @if($count > 1 and $superadmin_active > 1)
                                                                        <a class="btn btn-xs bg-danger" wire:click="setDeactivate({{ $superadmin->id }})"><i class="fas fa-user-xmark" style="cursor: pointer"></i></a>
                                                                    @endif
                                                                @endif
                                                            @else
                                                                @if($superadmin->status == "INACTIVE" or $superadmin->status == "LOCKED")
                                                                    <a class="btn btn-xs bg-success" wire:click.prevent="setActivate({{ $superadmin->id }})"><i class="fas fa-user-check" style="cursor: pointer"></i></a>
                                                                @endif
                                                            @endif

                                                            @if($superadmin->id == $my_id)
                                                                <a class="btn btn-xs bg-warning" href="{{ route('profile.show') }}"><i class="fas fa-signature" style="cursor: pointer"></i></a>
                                                            @endif
                                                        </td>
                                                    </tr>                                                    
                                                @endforeach
                                        ...
                                        <div class="mt-3">
                                            {{ $superadmins->links(data: ['scrollTo' => false]) }}
                                        </div>

                                        <div class="col-12 mt-3 text-right"><a class="text-success">Active: {{ $superadmin_active }}</a> | <a class="text-danger">Inactive: {{ $superadmin_inactive }}</a> | <a class="text-warning">Locked: {{ $superadmin_locked }}</a></div>
                                    </div>
                                </div>
                            </div>
                        @endif
...
@script
    <script>
        ...
        $('#nav-users-superadmin').addClass('active');
        ...
        Livewire.on('triggerDeactivate', s_id => {
            Swal.fire({
                title: 'Are you sure?',
                text: "Your account will be DEACTIVATED and you will automatically be logged out upon confirmation. Reactivation will require a superadmin approval.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3d9970',
                cancelButtonColor: '#d33',
        ...
        Livewire.on('triggerSuperRemove', s_id => {
            Swal.fire({
                title: 'Are you sure?',
                text: "This account will be REMOVED AS A SUPERADMIN upon confirmation.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3d9970',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, remove a superadmin!'
            }).then((result) => {
                if (result.isConfirmed) {
                    @this.set('confirm_super_remove', true);
                }
            });
        });
    </script>
