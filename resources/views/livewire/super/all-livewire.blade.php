                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mt-3">
                            <div class="card card-dark">
                                <div class="card-header">
                                    <h3 class="card-title"><i class="fas fa-user-gear mr-3"></i>Profile View</h3>
                        ...
                        @elseif($show_edit == true)
                            <div class="col-md-12 mt-3">
                                <div class="card card-blue">
                                    <div class="card-header">
                                        <h3 class="card-title"><i class="fas fa-user-gear mr-3"></i>Profile Update</h3>
                        ...
                        @else
                            <div class="col-md-12 mt-3">
                                <div class="card card-dark">
                                    <div class="card-header">
                                        <h3 class="card-title"><i class="fas fa-user-gear mr-3"></i>All Users ({{ $count }})</h3>
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if($admin_issuer < 1)
                                            <span class="badge badge-danger mb-3"><i class="fas fa-info-circle mr-1"></i>No issuer has been selected and set.</span>
                                        @else
                                        ...
                                        <table class="table table-sm table-responsive-sm table-hover">
                                            <thead class="bg-blue">
                                                <tr>
                                                    ...
                                                    <th width="12%"><i class="fas fa-tasks"></i></th>
                                                </tr>
                                            </thead>
                                            ...
                                            <tbody>
                                                @foreach($allusers as $alluser)
                                                    <tr>
                                                        <td class="text-sm" align="right">{{ $loop->iteration }}</td>
                                                        <td align="center"><img src="{{ $alluser->profile_photo_url }}" class="img-circle" width="25" height="25"></td>
                                                        <td class="text-sm">
                                                            @if($alluser->issuer_level == 'YES')
                                                                <b><i>{{ $alluser->name }}</i></b>
                                                            @else
                                                                {{ $alluser->name }}
                                                            @endif
                                                        </td>
                                                        <td class="text-sm">
                                                            @if($alluser->role == 'Superadmin')
                                                                <i class="fas fa-user-secret mr-1 text-gray text-sm"></i>{{ $alluser->role }}
                                                            @elseif($alluser->role == 'Administrator')
                                                                <i class="fas fa-user-tie mr-1 text-gray text-sm"></i>{{ $alluser->role }}
                                                            @else
                                                                <i class="fas fa-user mr-1 text-gray text-sm"></i>{{ $alluser->role }}
                                                            @endif
                                                        </td>
                                                        <td class="text-sm">{{ $alluser->email }}</td>
                                                        <td class="text-sm">{{ $alluser->office }} ({{ $alluser->office_full }})</td>
                                                        <td class="text-sm">{{ $alluser->position }} ({{ $alluser->position_full }})</td>
                                                        <td class="text-sm">
                                                            @if($alluser->status == "ACTIVE")
                                                                <div class="text-success"><i class="fas fa-user-check mr-1"></i>Active</div>
                                                            @elseif($alluser->status == "INACTIVE")
                                                        ...
                                                        <td>
                                                            <a class="btn btn-xs bg-primary" wire:click="viewAlluser({{ $alluser->id }})"><i class="fas fa-eye" style="cursor: pointer"></i></a>
                                                            <a class="btn btn-xs bg-purple" wire:click="editAlluser({{ $alluser->id }})"><i class="fas fa-building-user" style="cursor: pointer"></i></a>
                                                            
                                                            @if($alluser->status == "ACTIVE")
                                                                @if($alluser->role != 'Superadmin')
                                                                    <a class="btn btn-xs bg-danger" wire:click="setDeactivate({{ $alluser->id }})"><i class="fas fa-user-xmark" style="cursor: pointer"></i></a>
                                                                @endif
                                                            @else
                                                                <a class="btn btn-xs bg-success" wire:click.prevent="setActivate({{ $alluser->id }})"><i class="fas fa-user-check" style="cursor: pointer"></i></a>
                                                            @endif

                                                            @if($alluser->role == "User")
                                                                <a class="btn btn-xs bg-cyan" wire:click.prevent="setStaff({{ $alluser->id }})"><i class="fas fa-user-tie mr-1 ml-1" style="cursor: pointer"></i></a>
                                                                <a class="btn btn-xs bg-black" wire:click.prevent="setSuper({{ $alluser->id }})"><i class="fas fa-user-secret mr-1 ml-1" style="cursor: pointer"></i></a>
                                                            @endif

                                                            @if($alluser->supply_staff == "YES")
                                                                @if($alluser->issuer_level == "NO")
                                                                    <a class="btn btn-xs bg-pink" wire:click.prevent="setStaffRemove({{ $alluser->id }})"><i class="fas fa-user-minus" style="cursor: pointer"></i></a>
                                                                @endif
                                                            @endif

                                                            @if($admin_issuer < 1 and $alluser->supply_staff == "YES")
                                                                <a class="btn btn-xs bg-navy" wire:click.prevent="setIssuer({{ $alluser->id }})"><i class="fas fa-user-lock" style="cursor: pointer"></i></a>
                                                            @endif

                                                            @if($alluser->issuer_level == "YES")
                                                                <a class="btn btn-xs bg-gray" wire:click.prevent="setIssuerRemove({{ $alluser->id }})"><i class="fas fa-user-slash" style="cursor: pointer"></i></a>
                                                            @endif

                                                            @if($alluser->status == "LOCKED")
                                                                <a class="btn btn-xs bg-maroon" wire:click.prevent="setDelete({{ $alluser->id }})"><i class="fas fa-trash-alt mr-1 ml-1" style="cursor: pointer"></i></a>
                                                            @endif
                                                        </td>
                                                    </tr>                                                    
                                                @endforeach
                                        ...
                                        <div class="col-12 mt-3 text-right"><a class="text-success">Active: {{ $alluser_active }}</a> | <a class="text-danger">Inactive: {{ $alluser_inactive }}</a> | <a class="text-warning">Locked: {{ $alluser_locked }}</a></div>
                                    </div>
                                </div>
                            </div>
                        @endif
...
@script
    <script>
        ...
        Livewire.on('triggerStaff', s_id => {
            Swal.fire({
                title: 'Are you sure?',
                text: "This account will be SET AS AN ADMINISTRATOR upon confirmation.",
                icon: 'info',
                showCancelButton: true,
                confirmButtonColor: '#3d9970',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, set this account as an administrator!'
            }).then((result) => {
                if (result.isConfirmed) {
                    @this.set('confirm_staff', true);
                }
            });
        });
        ...
        Livewire.on('triggerSuper', s_id => {
            Swal.fire({
                title: 'Are you sure?',
                text: "This account will be SET AS A SUPERADMIN upon confirmation.",
                icon: 'info',
                showCancelButton: true,
                confirmButtonColor: '#3d9970',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, set this account as a superadmin!'
            }).then((result) => {
                if (result.isConfirmed) {
                    @this.set('confirm_super', true);
                }
            });
        });

        Livewire.on('triggerDelete', s_id => {
            Swal.fire({
                title: 'Are you sure?',
                text: "This account request will be DELETED upon confirmation.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3d9970',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, delete this account request!'
            }).then((result) => {
                if (result.isConfirmed) {
